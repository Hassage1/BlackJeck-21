...
class Play:
    ...
    def blackjack(self):
        ...
        if self.player.value == 21 and self.dealer.value == 21:
            ...
            black_jack("ничья!", 500, 100, grey)
            time.sleep(4)
            self.play_or_exit()
        elif self.player.value == 21:
            gameDisplay.blit(show_dealer_card, (550, 200))
            black_jack("Ты выйграл!", 500, 100, green)
            time.sleep(4)
            self.play_or_exit()
        elif self.dealer.value == 21:
            gameDisplay.blit(show_dealer_card, (550, 200))
            black_jack("Ты проиграл!", 500, 100, red)
            time.sleep(4)
            self.play_or_exit()
    ...
    def deal(self):
        ...
        game_texts("Рука Дилера:", 500, 150)
    ...
    def hit(self):
        ...
        if self.player.value > 21:
            show_dealer_card = pygame.image.load('img/' + self.dealer.card_img[1] + '.png').convert()
            gameDisplay.blit(show_dealer_card, (550, 200))
            game_finish("ты проиграл!", 500, 100, red)
            time.sleep(3)
            self.play_or_exit()
    ...
    def stand(self):
        ...
        if self.player.value > self.dealer.value:
            game_finish("Ты выйграл!", 500, 100, green)
            time.sleep(4)
            self.play_or_exit()
        elif self.player.value < self.dealer.value:
            game_finish("Дилер выйграл!", 500, 100, red)
            time.sleep(4)
            self.play_or_exit()
        else:
            game_finish("ничья", 500, 100, grey)
            time.sleep(4)
            self.play_or_exit()
...
while running:
    for event in pygame.event.get():
        ...
        button("Сделка", 30, 100, 150, 50, light_slat, dark_slat, play_blackjack.deal)
        button("Еще карта", 30, 200, 150, 50, light_slat, dark_slat, play_blackjack.hit)
        button("Сравнить", 30, 300, 150, 50, light_slat, dark_slat, play_blackjack.stand)
        button("Выход", 30, 500, 150, 50, light_slat, dark_red, play_blackjack.exit)
